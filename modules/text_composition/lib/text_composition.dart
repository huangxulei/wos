...
class TextComposition extends ChangeNotifier {
  ...
  Future<void> init(
      void Function(List<AnimationController> _controller)
    ...
    _currentIndex = TOTAL * 12345 + HALF;
    final n =
        ((_initPercent * chapters.length - _firstChapterIndex) * pages.length)
            .round();
    if (n < 2) {
      _firstIndex = _currentIndex;
  ...
  List<Widget> get pages {
    ...
    return [
      for (var i = _currentIndex + HALF, last = _currentIndex - HALF;
          i > last;
          i--)
        CustomPaint(
  ...
  checkSave() {
    ...
    Future.delayed(saveDelay).then((value) {
      if (_disposed || DateTime.now().millisecondsSinceEpoch < _lastSaveTime)
        return;
  ...
  void turnPage(DragUpdateDetails details, BoxConstraints dimens,
    ...
    final _ratio =
        vertical ? (offset / dimens.maxHeight) : (offset / dimens.maxWidth);
    if (isForward == null) {
      if (offset > 0) {
        isForward = false;
      } else {
  ...
  Future<void> onDragFinish() async {
    ...
    if (isForward != null) {
      if (isForward!) {
        if (!_isLastPage &&
            _controllers[currentIndex % TOTAL].value <=
                (cutoffNext / 100 + 0.03)) {
          nextPage();
        } else {
  ...
  Future<void> nextChapter([bool animation = true]) async {
    if (_disposed ||
        _lastChapterIndex >= chapters.length - 1 ||
        _nextChapterLoading) return;
  ...
  Future<List<TextPage>> startX(int index) async {
    ...
    final _height =
        size.height - (config.showInfo ? 24 : 0) - config.bottomPadding;
